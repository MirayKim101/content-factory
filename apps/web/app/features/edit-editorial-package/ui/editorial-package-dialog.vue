    ...
    >
      ...
      <template v-else>
        ...
        <form @submit.prevent="requestSave">
          ...
          <div
            v-if="currentPackage?.revision.provenance"
            class="provenance"
            aria-label="Происхождение редакционных компонентов"
          >
            <span
              :title="currentPackage.revision.provenance.metadata.basisVersion"
              >Метаданные:
              {{ currentPackage.revision.provenance.metadata.mode }}</span
            >
            <span
              :title="currentPackage.revision.provenance.thumbnail.basisVersion"
              >Обложка:
              {{ currentPackage.revision.provenance.thumbnail.mode }}</span
            >
          </div>
