...
describe("creator context operation identity", () => {
  ...
  it("restores one idempotency key only for the exact operation, target and body", () => {
    ...
    expect(creatorOperationKey("source-context", "p:s:1", body)).toBe(
      "00000000-0000-4000-8000-000000000001",
    );
    expect(creatorOperationKey("source-context", "p:s:1", body)).toBe(
      "00000000-0000-4000-8000-000000000001",
    );
    expect(creatorOperationKey("source-context", "p:s:2", body)).toBe(
      "00000000-0000-4000-8000-000000000002",
    );
    expect(creatorOperationKey("cut-prompt", "p:s:1", body)).toBe(
      "00000000-0000-4000-8000-000000000003",
    );
    expect(
      creatorOperationKey("source-context", "p:s:1", {
        ...body,
        expectedRevision: 3,
      }),
  ...
  it("drops a completed operation identity", () => {
    ...
    expect(
      sessionStorage.getItem(
        "content-factory.creator-context.profile-update.profile",
      ),
    ).toBeNull();
  });
  ...
  it("fails closed when browser storage cannot persist a retry identity", () => {
    ...
    expect(() =>
      creatorOperationKey("profile-create", "new", { name: "A" }),
    ).toThrow(CreatorOperationStorageError);
  });
});
